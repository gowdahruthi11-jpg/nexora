...
async def generate_certificate(data: CertificateRequest):
    ...
    case_id = f"PR-ASCEND-{data.sha256_hash[:8].upper()}"
    timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")

    # Header band
    c.setFillColor(colors.HexColor("#1b0b3e"))
    c.rect(0, height - 90, width, 90, fill=True, stroke=False)
    c.setFillColor(colors.white)
    c.setFont("Helvetica-Bold", 19)
    c.drawString(50, height - 42, "Digital Evidence Verification Certificate")
    c.setFont("Helvetica", 10)
    c.drawString(50, height - 62, f"Case Reference: {case_id}")
    c.drawString(50, height - 76, f"Issued: {timestamp}")

    y = height - 125
    c.setFillColor(colors.black)

    c.setFont("Helvetica-Bold", 12)
    c.drawString(50, y, "File Under Review")
    y -= 20
    c.setFont("Helvetica", 11)
    c.drawString(50, y, f"Filename: {data.filename}")
    y -= 16
    c.drawString(50, y, f"Type: {data.file_type}")
    y -= 16
    c.setFont("Helvetica", 8.5)
    c.drawString(50, y, f"SHA-256: {data.sha256_hash}")

    y -= 38
    c.setFont("Helvetica-Bold", 12)
    c.drawString(50, y, "Trust Assessment")
    y -= 32
    score_color = colors.HexColor("#2e9e5b") if data.trust_score >= 80 else (
        colors.HexColor("#c9922f") if data.trust_score >= 50 else colors.HexColor("#c1584f"))
    c.setFillColor(score_color)
    c.setFont("Helvetica-Bold", 30)
    c.drawString(50, y, f"{data.trust_score}/100")
    c.setFillColor(colors.black)
    c.setFont("Helvetica-Bold", 13)
    c.drawString(185, y + 9, data.verdict)
    ...
    y -= 20
    c.setFont("Helvetica", 10)
    for item in data.score_breakdown:
        wrapped = textwrap.wrap(item, 95)
        for line in wrapped:
            if y < 90:
                c.showPage()
                y = height - 60
    ...
    c.drawString(50, 38, "Generated by Verity — PR-ASCEND, NEXORA 2026 Innovation Hackathon.")
    c.drawString(50, 26, "Automated forensic-style analysis for demonstration purposes; not a legal instrument.")
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename=verification_certificate_{data.sha256_hash[:8]}.pdf"}
    )
